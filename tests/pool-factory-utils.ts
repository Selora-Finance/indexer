...
export function createPoolCreatedEvent(
    ...
    poolCreatedEvent.parameters.push(new ethereum.EventParam('stable', ethereum.Value.fromBoolean(stable)));
    poolCreatedEvent.parameters.push(new ethereum.EventParam('pool', ethereum.Value.fromAddress(pool)));
    poolCreatedEvent.parameters.push(new ethereum.EventParam('param4', ethereum.Value.fromUnsignedBigInt(param4)));

    return poolCreatedEvent;
}

export function createSetCustomFeeEvent(pool: Address, fee: BigInt): SetCustomFee {
    let setCustomFeeEvent = changetype<SetCustomFee>(newMockEvent());

    setCustomFeeEvent.parameters = new Array();

    setCustomFeeEvent.parameters.push(new ethereum.EventParam('pool', ethereum.Value.fromAddress(pool)));
    setCustomFeeEvent.parameters.push(new ethereum.EventParam('fee', ethereum.Value.fromUnsignedBigInt(fee)));
...
export function createSetFeeManagerEvent(feeManager: Address): SetFeeManager {
    ...
    setFeeManagerEvent.parameters.push(new ethereum.EventParam('feeManager', ethereum.Value.fromAddress(feeManager)));
...
export function createSetPauseStateEvent(state: boolean): SetPauseState {
    ...
    setPauseStateEvent.parameters.push(new ethereum.EventParam('state', ethereum.Value.fromBoolean(state)));
...
export function createSetPauserEvent(pauser: Address): SetPauser {
    ...
    setPauserEvent.parameters.push(new ethereum.EventParam('pauser', ethereum.Value.fromAddress(pauser)));
...
export function createSetVoterEvent(voter: Address): SetVoter {
    ...
    setVoterEvent.parameters.push(new ethereum.EventParam('voter', ethereum.Value.fromAddress(voter)));
